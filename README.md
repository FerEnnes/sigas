# 🌿 SaaS Agro Light

Sistema web para gestão agrícola de pequenas propriedades e agricultura familiar com painel financeiro, calendário inteligente e organização de propriedades e fornecedores.

<img width="2048" alt="Captura de Tela 2025-03-31 às 01 29 41" src="https://github.com/user-attachments/assets/535a75f2-68fb-4055-9aae-ecd8a0b48202" />


## 🧩 Funcionalidades

- 📊 **Dashboard** com indicadores, clima e resumo geral
- 👥 **Cadastros**:
  - Usuários
  - Clientes
  - Fornecedores
  - Propriedades rurais
- 🧾 **Plano de Contas** integrado
- 🗓️ **Calendário Agrícola** com:
  - Eventos
  - Lembretes
  - Tarefas
- 🔔 **Notificações inteligentes**
- ⚙️ **Configurações gerais**
- 💡 Interface moderna, responsiva e intuitiva

---

## 🚀 Tecnologias Utilizadas

- React.js
- React Router DOM
- Material UI (MUI)
- React Big Calendar
- Moment.js
- React Helmet Async
- ViaCEP API (CEP automático)

---

## 📦 Como rodar o projeto

```bash
git clone https://github.com/FerEnnes/saas-agro-light.git
cd saas-agro-light
npm install
npm start

### `npm run build` fails to minify

This section has moved here: [https://facebook.github.io/create-react-app/docs/troubleshooting#npm-run-build-fails-to-minify](https://facebook.github.io/create-react-app/docs/troubleshooting#npm-run-build-fails-to-minify)